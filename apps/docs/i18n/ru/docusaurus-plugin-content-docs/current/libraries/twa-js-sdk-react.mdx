...
Внутри, `SDKProvider` использует функцию `init` из `@twa.js/sdk`. Она принимамет опциональный список параметров при помощи свойства `initOptions`, описанного [здесь](twa-js-sdk/about#init).
